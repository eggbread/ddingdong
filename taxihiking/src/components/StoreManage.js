...
class StoreManage extends Component {
  ...
  componentWillMount() {
    axios
      ...
      .then(res => {
        if (res.data) {
          axios
            .post("http://localhost:4000/storemanage", {
              token: window.sessionStorage.getItem("token")
            })
            .then(res => {
  ...
  open(e) {
    ...
    if (this.state.addOpen) {
      ...
      $("#storemenu").append(
        JSON.parse(clickObj.menu).map(
          (item, index) =>
            '<li><img width="150px" height="150px"src=' +
            require("../asset/images/" + clickObj.userid + "/" + item.img) +
            '></img><input type="file" name="menuImg" class="menuImg"></input><br/><input value="' +
            item.name +
            '" class="key"></input>  <input value="' +
            item.price +
            '" class="value"></input><button type="button" class="removebtn"  onclick=this.parentElement.remove()>삭제</button></button></li>'
        )
      );
      ...
      $('[name="storeimg"]').attr(
        "src",
        require("../asset/images/" + clickObj.userid + "/main.jpg")
      );
    } else {
      $("#subbtn").html("추가하기");
      $("#subbtn").val("추가하기");
  ...
  addbtn() {
    $("#storemenu").append(
      '<li class="storemenu"><input type="file" name="menuImg" class="menuImg"></input><br/><input type="text" class="key"></input><input type="text" class="value"></input><button type="button" onclick=this.parentElement.remove()>삭제</button></li>'
    );
  }
  ...
  makeorderList() {
    ...
    Object.keys(temp).map(item =>
      $("#receiveOrderList").append(
        '<li name="storemenu">이름 : ' +
          temp[item].menuname +
          ", 개수 : " +
          temp[item].menumany +
          "</li>"
  ...
  sendorderList() {
    ...
    for (var i = 0; i < count; i++) {
      ...
      if (document.getElementsByClassName("menuImg")[i].value) {
        fix[i] = true;
        jsonFormat["img"] = document.getElementsByClassName("menuImg")[
          i
        ].files[0].name;
      } else {
        fix[i] = false;
                ...
                <TableBody>
                  <div>
                    ...
                    <TableRow>
                      ...
                      <TableCell>{item.openinghours}</TableCell>
                      <br />
                      <br />
                    </TableRow>
                    <TableRow>
                  ...
                  <div>
                    ...
                    <TableRow>
                      <TableCell>
                        <img
                          src={
                            " https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=ddingdong.gq/menu/" +
                            item.storeID
                          }
                          alt=""
                        />
      ...
      return (
        <div>
          ...
          <Modal
            ...
            ariaHideApp={false}
            onAfterClose={this.close.bind(this)}
            style={customStyles}
          >
            <Button
              id="Button"
            ...
            <span id="span">Store Profile</span>
            <br />
            <form
              className="form"
              action="http://localhost:4000/storemanage/fix"
              method="POST"
            ...
            >
              ...
              <p>
                <Button
                  ...
                  variant="light"
                  type="button"
                  onClick={this.postOpen.bind(this)}
                  id="storelocation"
                >
                  주소 찾기
                </Button>
              ...
              <ul id="storemenu" name="storemenu" />
              <p>
                메인 사진 :{" "}
                <input
                  type="file"
              ...
              <img src="" alt="" name="storeimg" width="150px" height="150px" />{" "}
              <br />
              <Button
                type="submit"
                id="subbtn"
          ...
          >
            ...
            <h1>주문이 도착하였습니다</h1>
            <p>
              {this.state.orderList.tableNumber}번 테이블에서 주문이
              도착하였습니다.
            </p>
            <div>
              주문목록
              <ul id="receiveOrderList" />
            </div>
            <div>
              <p>특별 주문입니다</p>
              {this.state.orderList.special}
            </div>
